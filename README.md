...
### Latest fixes
- Fixed DB password initialization so `DB_PASS` is generated before database setup.
- Domain input accepts `https://` and normalizes it to the hostname internally.
- Updated the installer menu to a cleaner professional SKN Paymenter Deployer layout.
- The installer confirms all details once and reuses them during setup.
