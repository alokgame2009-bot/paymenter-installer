...
prompt_config() {
  ...
  while [[ -z "${ADMIN_PASS:-}" ]]; do
    ...
    echo
    [[ -n "$ADMIN_PASS" ]] || warn "Password cannot be empty."
  done
...
run_interactive_init() {
  ...
  # Create/update the account directly through Laravel so no TTY or
  # Laravel Prompts menu is needed.
  ...
  sudo -u www-data env HOME=/tmp XDG_CONFIG_HOME=/tmp/paymenter-psysh \
    ...
    php artisan tinker --execute='$first=getenv("SKN_ADMIN_FIRST_NAME"); $last=getenv("SKN_ADMIN_LAST_NAME"); $email=strtolower(trim(getenv("SKN_ADMIN_EMAIL"))); $pass=getenv("SKN_ADMIN_PASS"); $roleName=getenv("SKN_USER_ROLE"); $user=\App\Models\User::where("email",$email)->first(); if(!$user){$user=new \App\Models\User();} $user->first_name=$first; $user->last_name=$last; $user->name=trim($first." ".$last); $user->email=$email; if($pass!==""){$user->password=\Illuminate\Support\Facades\Hash::make($pass);} $user->save(); if(method_exists($user,"syncRoles")){$user->syncRoles([$roleName]);} elseif(method_exists($user,"assignRole")){$user->assignRole($roleName);} echo "SKN_ADMIN_CREATED\n";'
...
show_menu() {
  ...
  case "$MENU_CHOICE" in
    ...
    *) printf '%b\n' "${C1}Invalid option. Please select 1, 2, or 3.${NC}"; sleep 1; show_menu ;;
  esac
}
