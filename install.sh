...
uninstall() {
  ...
  if [[ "$confirm" != "UNINSTALL" ]]; then
    warn "Uninstall cancelled. Returning to main menu..."
    sleep 1
    return 0
  fi
...
prompt_config() {
  ...
  printf '\033[38;5;51m║                 SKN PAYMENTER INSTALLER                    ║\033[0m\n'
  printf '\033[38;5;51m║                    INSTALLATION SETUP                      ║\033[0m\n'
  printf '\033[38;5;51m╚══════════════════════════════════════════════════════════════╝\033[0m\n'
  echo
  echo 'Enter your Paymenter details. Invalid values will ask again instead of exiting.'
  echo

  while true; do
    read -rp 'Domain: ' v || exit 0
    v="${v#http://}"; v="${v#https://}"; v="${v%%/*}"
    if [[ "$v" =~ ^[A-Za-z0-9.-]+$ && "$v" == *.* && "$v" != .* && "$v" != *. ]]; then DOMAIN="$v"; break; fi
    warn 'Invalid domain. Example: billing.skylernodes.fun'
  done

  while true; do
    read -rp 'Company Name: ' v || exit 0
    v="$(echo "$v" | xargs)"
    if [[ -n "$v" ]]; then COMPANY="$v"; break; fi
    warn 'Company name cannot be empty. Please enter it again.'
  done

  while true; do
    read -rp 'First Name: ' v || exit 0
    v="$(echo "$v" | xargs)"
    if [[ "$v" =~ ^[A-Za-z][A-Za-z[:space:]]*$ ]]; then ADMIN_FIRST_NAME="$v"; break; fi
    warn 'Invalid first name. Please enter it again.'
  done

  while true; do
    read -rp 'Last Name: ' v || exit 0
    v="$(echo "$v" | xargs)"
    if [[ "$v" =~ ^[A-Za-z][A-Za-z[:space:]]*$ ]]; then ADMIN_LAST_NAME="$v"; break; fi
    warn 'Invalid last name. Please enter it again.'
  done

  while true; do
    read -rp 'Email Address: ' v || exit 0
    v="$(echo "$v" | xargs)"
    if [[ "$v" =~ ^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$ ]]; then ADMIN_EMAIL="${v,,}"; break; fi
    warn 'Invalid email address. Please enter it again.'
  done

  while true; do
    read -rsp 'Account Password: ' v || exit 0
    echo
    if [[ ${#v} -ge 8 ]]; then ADMIN_PASS="$v"; break; fi
    warn 'Password must be at least 8 characters. Please enter it again.'
  done

  while true; do
    read -rp 'Account Type (admin/user) [admin]: ' v || exit 0
    v="${v:-admin}"; v="${v,,}"; v="$(echo "$v" | xargs)"
    if [[ "$v" == 'admin' || "$v" == 'user' ]]; then ACCOUNT_TYPE="$v"; break; fi
    warn 'Invalid account type. Enter admin or user.'
  done

  while true; do
    read -rp "Let's Encrypt Email: " v || exit 0
    v="$(echo "$v" | xargs)"
    if [[ "$v" =~ ^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$ ]]; then LE_EMAIL="${v,,}"; break; fi
    warn "Invalid Let's Encrypt email. Please enter it again."
  done
  ...
  echo
  printf '\033[38;5;51m╔══════════════════════════════════════════════════════════════╗\033[0m\n'
  printf '\033[38;5;51m║                    INSTALLATION DETAILS                    ║\033[0m\n'
  printf '\033[38;5;51m╠══════════════════════════════════════════════════════════════╣\033[0m\n'
  printf '\033[38;5;45m║ %-18s : %-38s ║\033[0m\n' 'Domain' "https://${DOMAIN}"
  printf '\033[38;5;39m║ %-18s : %-38s ║\033[0m\n' 'Company' "$COMPANY"
  printf '\033[38;5;51m║ %-18s : %-38s ║\033[0m\n' 'First name' "$ADMIN_FIRST_NAME"
...
SQL
  ...
  sudo -u www-data php artisan db:seed --class=CustomPropertySeeder --force
...
EOF
  ...
  echo
  printf '[38;5;51m✓ Installation finished successfully. Returning to main menu...[0m\n'
  sleep 2
  show_menu
}

loading() {
  local text="${1:-Working}" i=0
  local frames=('⠋' '⠙' '⠹' '⠸' '⠼' '⠴' '⠦' '⠧' '⠇' '⠏')
  printf '\033[?25l'
  while (( i < 10 )); do
    printf '\r\033[38;5;51m%s %s\033[0m' "${frames[i]}" "$text"
    sleep 0.08
    ((i++))
  done
  printf '\r\033[K'
...
show_menu() {
  ...
  printf '%b\n' "${C3}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo
  printf '%b' "${C4}   Select an option [1-3]: ${NC}"
  read -r MENU_CHOICE

  case "$MENU_CHOICE" in
    1)
      loading "Starting Paymenter installation..."
      install
      ;;
    2)
      ...
      ;;
    3)
      printf '[38;5;51m✓ Goodbye.[0m
'
      exit 0
      ;;
    *)
      printf '[38;5;214m⚠ Invalid option. Please select 1, 2, or 3.[0m
'
      sleep 1
      show_menu
      ;;
